...
cdef void _k_closest_on_chunk(
    const np.float64_t *X_train_c,              # IN
    const np.float64_t *X_test_c,               # IN
    const np.float64_t[::1] X_train_sq_norms,   # IN
    const np.float64_t *dist_middle_terms,      # IN
    np.float64_t *heaps_red_distances,          # IN/OUT
    integral *heaps_indices,                    # IN/OUT
    integral k,                                 # IN
    # Shape of X_test_c
    integral X_test_c0,                         # IN
    integral X_test_c1,                         # IN
    # Shape of X_train_c (X_train_c1 = X_test_c1)
    integral X_train_c0,                        # IN
    # ID of the first element of X_train_c
    integral X_train_idx_offset,                # IN
) nogil:
    cdef:
        integral i, j
    # Instead of computing the full pairwise squared distances matrix,
    # ||X_test_c - X_train_c||² = ||X_test_c||² - 2 X_test_c.X_train_c^T + ||X_train_c||², we only need
    # to store the - 2 X_test_c.X_train_c^T + ||X_train_c||² term since the argmin
    # for a given sample X_test_c^{i} does not depend on ||X_test_c^{i}||²
    ...
    _gemm(RowMajor, NoTrans, Trans,
          X_test_c0, X_train_c0, X_test_c1,
          -2.0,
          X_test_c, X_test_c1,
          X_train_c, X_test_c1, 0.0,
          dist_middle_terms, X_train_c0)

    # Computing argmins here
    for i in range(X_test_c0):
        for j in range(X_train_c0):
            _push(heaps_red_distances + i * k,
                  heaps_indices + i * k,
                  k,
                  # reduced distance: - 2 X_test_c_i.X_train_c_j^T + ||X_train_c_j||²
                  dist_middle_terms[i * X_train_c0 + j] + X_train_sq_norms[j],
...
cdef int _parallel_knn_on_X_test(
    ...
    np.float64_t[:, ::1] knn_red_distances,   # OUT
) nogil except -1:
    cdef:
        integral k = knn_indices.shape[1]
        integral d = X_test.shape[1]
        integral sf = sizeof(np.float64_t)
        ...
        np.float64_t *dist_middle_terms_chunks
        np.float64_t *heaps_red_distances_chunks

        np.float64_t *X_train_c_upcasted
        np.float64_t *X_test_c_upcasted


    with nogil, parallel(num_threads=num_threads):
        # Thread local buffers

        X_train_c_upcasted = <np.float64_t *> malloc(
            X_train_n_samples_chunk * d * sf)

        X_test_c_upcasted = <np.float64_t *> malloc(
            X_test_n_samples_chunk * d * sf)

        # Temporary buffer for the -2 * X_test_c.dot(X_train_c.T) term
        dist_middle_terms_chunks = <np.float64_t*> malloc(
            X_train_n_samples_chunk * X_test_n_samples_chunk * sf)
        heaps_red_distances_chunks = <np.float64_t*> malloc(
        ...
        for X_test_chunk_idx in prange(X_test_n_chunks, schedule='static'):
            ...
            for idx in range(X_test_end - X_test_start):
                for jdx in range(d):
                    X_test_c_upcasted[idx * d + jdx] = (
                        <np.float64_t> X_test[X_test_start + idx, jdx]
                    )
            ...
            for X_train_chunk_idx in range(X_train_n_chunks):
                ...
                _k_closest_on_chunk(
                    ...
                    k,
                    # Shape of X_test_c
                    X_test_end - X_test_start,
                    d,
                    # Shape of X_train_c
                    X_train_end - X_train_start,
                    # X_train_idx_offset
                    X_train_start,
                )
        ...
        free(X_test_c_upcasted)
        free(X_train_c_upcasted)
...
cdef int _parallel_knn_on_X_train(
    ...
    cdef:
        ...
        integral X_train_start, X_train_end, X_test_start, X_test_end
        integral X_test_chunk_idx, X_train_chunk_idx, idx, jdx, kdx, ldx
    ...
    for X_test_chunk_idx in range(X_test_n_chunks):
        ...
        with nogil, parallel(num_threads=num_threads):
            ...
            for X_train_chunk_idx in prange(X_train_n_chunks, schedule='static'):
                ...
                for kdx in range(X_train_end - X_train_start):
                    for ldx in range(d):
                        X_train_c_upcasted[kdx * d + ldx] = (
                            <np.float64_t> X_train[X_train_start + kdx, ldx]
                        )
...
def parallel_knn(
    ...
    cdef:
        integral[:, ::1] knn_indices = np.full((X_test.shape[0], k), 0,
                                               dtype=int_dtype)
        np.float64_t[:, ::1] knn_red_distances = np.full((X_test.shape[0], k),
                                                     FLOAT_INF,
                                                     dtype=np.float64)
        np.float64_t[:, ::1] X_train_upcasted = np.asarray(X_train).astype(np.float64)
        np.float64_t[::1] X_train_sq_norms = np.einsum('ij,ij->i',
                                                       X_train_upcasted,
                                                       X_train_upcasted)
